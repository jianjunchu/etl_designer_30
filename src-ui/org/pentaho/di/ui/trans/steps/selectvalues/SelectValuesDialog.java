...
public class SelectValuesDialog extends BaseStepDialog implements StepDialogInterface
{
	...
	public String open()
	{
		...
		final int FieldsCols=6;//tony 20171107
	...
	{	
		...
		if (input.getSelectName()!=null && input.getSelectName().length>0)
		{
			for (int i=0;i<input.getSelectName().length;i++)
			{
				...
				if (input.getFunctionExpression()[i]!=null)
					item.setText(5, input.getFunctionExpression()[i]); //$NON-NLS-1$ //$NON-NLS-2$
				if (input.getPadChar()[i]!=null)
					item.setText(6, input.getPadChar()[i]); //Tony 20171107
			}
			wFields.setRowNums();
  ...
  private String[] getCharsets()
  {
	...
	private void ok()
	{
		...
		for (int i=0;i<nrfields;i++)
		{
			...
			input.getFunctionExpression()        [i] = item.getText(5);
			input.getPadChar() [i] = item.getText(6); //Tony 20171106
		}
