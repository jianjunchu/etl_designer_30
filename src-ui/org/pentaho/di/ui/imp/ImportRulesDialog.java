...
public class ImportRulesDialog extends Dialog implements XulEventHandler {
  ...
  public boolean open() {
    ...
    enabledColumn.setText(BaseMessages.getString(PKG,"ImportRulesDialog.on"));
    enabledColumn.setWidth(50);

    // Rule description column
    //
    TableColumn descriptionColumn = new TableColumn(table, SWT.NONE);
    descriptionColumn.setText(BaseMessages.getString(PKG,"ImportRulesDialog.RuleDescription"));
    descriptionColumn.setWidth(400);

    // Rule description column
    //
    TableColumn compositeColumn = new TableColumn(table, SWT.NONE);
    compositeColumn.setText(BaseMessages.getString(PKG,"ImportRulesDialog.Settings"));
  ...
  public void addRule() {
    ...
    if (plugins.size()>0) {
      ...
      EnterSelectionDialog esd = new EnterSelectionDialog(shell, names, BaseMessages.getString(PKG,"ImportRulesDialog.SelectARule"), BaseMessages.getString(PKG,"ImportRulesDialog.SelectAdd"));
  ...
  public void removeRule() {
    ...
    box.setText(BaseMessages.getString(PKG,"ImportRulesDialog.Warning"));
    box.setMessage(BaseMessages.getString(PKG,"ImportRulesDialog.RemoveSelectedRules"));
    int answer = box.open();
    if (answer!=SWT.YES){
      return;
    }
  ...
  protected void exportRules() {
    ...
    if (dialog.open() != null)
    {
      ...
      try {
        ...
        new ErrorDialog(shell, BaseMessages.getString(PKG,"ImportRulesDialog.Error"), "There was an error while exporting to file '"+filename+"'", e);
      } finally {
        try { fileWriter.close();
        } catch( Exception e) {
          new ErrorDialog(shell, BaseMessages.getString(PKG,"ImportRulesDialog.Error"), "There was an error closing file '"+filename+"'", e);
        }
      }
  ...
  protected void importRules() {
    if (!importRules.getRules().isEmpty()) {
      MessageBox box = new MessageBox(shell, SWT.ICON_WARNING | SWT.APPLICATION_MODAL | SWT.YES | SWT.NO);
      box.setText(BaseMessages.getString(PKG,"ImportRulesDialog.Warning"));
      box.setMessage(BaseMessages.getString(PKG,"ImportRulesDialog.LoadNewSet"));
      int answer = box.open();
      if (answer!=SWT.YES){
        return;
      }
