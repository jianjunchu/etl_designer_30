...
public class KettleDatabaseRepositoryConnectionDelegate extends KettleDatabaseRepositoryBaseDelegate {
    ...
    //add by cli
    public synchronized ObjectId getNextCacheFileID()throws KettleException
    {
    	return getNextID(quoteTable(KettleDatabaseRepository.TABLE_R_CACHE_FILE), quote(KettleDatabaseRepository.FIELD_ID_CACHE_FILE));
    }
    ...
    public synchronized List<RepositoryElementMetaInterface> getRepositoryObjects(String tableName, RepositoryObjectType objectType, ObjectId id_directory) throws KettleException
    {
    	try
    	{
	    	...
	    	//get all resource ID edit by cli
	    	String resourceIDs="";
	    	if(KettleDatabaseRepositoryMeta.priviledge>0){
		    	int userID = Integer.parseInt(this.repository.getRepositoryMeta().getUserID());
		    	String resourceSql = "select * from "+KettleDatabaseRepository.TABLE_KDI_T_USER_RESOURCE+" where "
					+ KettleDatabaseRepository.FIELD_C_USER_ID+" = "+userID+" and "+KettleDatabaseRepository.FIELD_C_RESOURCE_TYPE_ID
	  				+"= "+resourceType;
		    	ResultSet resourceRs = database.openQuery(resourceSql);
		    	while(resourceRs.next()){
		    		resourceIDs = resourceIDs+"'"+resourceRs.getInt(KettleDatabaseRepository.FIELD_C_RESOURCE_ID)+"',";
		    	}	    		
	    	}
	        ...
	        if (rs != null)
	        {
	    		...
	    		for (Object[] r : rows)
	            {
	    			if(KettleDatabaseRepositoryMeta.priviledge>0){ //filt id, edit by cli
		    			if(resourceIDs.contains("'"+r[4].toString()+"'")){
	    			...
	    			}else{//管理员用户可以看到所有的资源 edit by cli
	    				ObjectId id = new LongObjectId( rowMeta.getInteger(r, 4) );
			            repositoryObjects.add(new RepositoryObject(id, rowMeta.getString(r, 0), repositoryDirectory, rowMeta.getString(r, 1), rowMeta.getDate(r, 2), objectType, rowMeta.getString(r, 3), false));	    				
    ...
    public Collection<RowMetaAndData> getDatabaseAttributes(ObjectId id_database) throws KettleDatabaseException, KettleValueException {
      ...
      try {
        ...
        for (Object[] row : rows) {//
          ...
          long id = 0;
          //xnren start
          //rep database set to kingbase es,do not add quote to the column
          if("KingbaseES".equals(databaseMeta.getDatabaseInterface().getPluginName())){
        	  id = rowWithMeta.getInteger(KettleDatabaseRepository.FIELD_DATABASE_ATTRIBUTE_ID_DATABASE_ATTRIBUTE, 0);
          }else {
        	  id = rowWithMeta.getInteger(quote(KettleDatabaseRepository.FIELD_DATABASE_ATTRIBUTE_ID_DATABASE_ATTRIBUTE), 0);
          }
