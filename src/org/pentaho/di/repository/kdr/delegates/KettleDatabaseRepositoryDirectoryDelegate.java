...
public class KettleDatabaseRepositoryDirectoryDelegate extends KettleDatabaseRepositoryBaseDelegate {
		...
		try {
			synchronized (repository) {
				if(KettleDatabaseRepositoryMeta.priviledge>0){
					...
					int userID = Integer.parseInt(repository.getRepositoryMeta().getUserID());
					  String sql = "select * from "+KettleDatabaseRepository.TABLE_KDI_T_USER_RESOURCE+" where "
					...
					String dirIDs = "";
					if (rs.next()) {
						dirIDs =dirIDs+"'"+ rs.getInt("C_RESOURCE_ID")+"'";
						while (rs.next()) {
							dirIDs =dirIDs + ",'" + rs.getInt("C_RESOURCE_ID")+"'";
						}
						...
						for (int i = 0; i < subids.length; i++) {
							if(dirIDs.contains("'"+subids[i]+"'")){ //edit by cli ,add if condition
								RepositoryDirectory subdir = new RepositoryDirectory();
								loadRepositoryDirectory(subdir, subids[i],dirIDs);
			...
			"An error occured loading the directory tree from the repository",e);
		}
	}
  ...
  public void loadRepositoryDirectory(RepositoryDirectory repositoryDirectory, ObjectId id_directory)throws KettleException{
	  loadRepositoryDirectory(repositoryDirectory,id_directory,"unlimit");
  }

	public void loadRepositoryDirectory(
			RepositoryDirectory repositoryDirectory, ObjectId id_directory,
			String dirIDs) throws KettleException {
		if (id_directory == null) {
			// This is the root directory, id = OL
			id_directory = new LongObjectId(0L);
		}
		//edit by cli  add if condition
		String dirId = "'" + id_directory.getId() + "'";
		if (dirIDs.contains(dirId)||"unlimit".equals(dirIDs)) {
			try {
				...
				if (row != null) {
					...
					for (int i = 0; i < subids.length; i++) {
						if((!"unlimit".equals(dirIDs))&&dirIDs.contains("'"+subids[i]+"'")){ //edit by cli add if condition
						RepositoryDirectory subdir = new RepositoryDirectory();
						loadRepositoryDirectory(subdir, subids[i], dirIDs);
						repositoryDirectory.addSubdirectory(subdir);
						}
						else if("unlimit".equals(dirIDs)){
							RepositoryDirectory subdir = new RepositoryDirectory();
							loadRepositoryDirectory(subdir, subids[i], dirIDs);
							repositoryDirectory.addSubdirectory(subdir);							
						}
				...
				throw new KettleException(
						BaseMessages.getString(PKG,"Repository.LoadRepositoryDirectory.ErrorLoading.Exception"),e);
			}
		}
	}
