 ...
 */
public class StringCutMeta extends BaseStepMeta implements StepMetaInterface, StepMetaInjectionInterface {
      ...
      for (StepInjectionMetaEntry entry : metadata) {
        ...
        if (entry.getValueType()!=ValueMetaInterface.TYPE_NONE) {
          ...
          if (attr.getKey().equals("FIELDS")) {
            ...
            allocate(inputFieldEntries.size());
            ...
            for (int row=0;row<inputFieldEntries.size();row++) {
              ...
              for (int i=0;i<fieldAttributes.size();i++) {
                ...
                if (fieldAttr.getKey().equals("in_stream_name")) { fieldInStream[row]=attributeValue; } else 
                if (fieldAttr.getKey().equals("out_stream_name")) { fieldOutStream[row]=attributeValue; } else 
                if (fieldAttr.getKey().equals("cut_from")) { cutFrom[row] = (attributeValue==null?"-1":attributeValue); } else 
                if (fieldAttr.getKey().equals("cut_to")) { cutTo[row] = (attributeValue==null?"-1":attributeValue); } else 
                {
