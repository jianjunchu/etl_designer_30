...
public class CheckSum extends BaseStep implements StepInterface {
	...
	private byte[] createBinaryCheckSum(Object[] r) throws Exception {
...
//		 int aryLen = 0;
		...
		// Loop through fields
		for (int i = 0; i < data.fieldnr; i++) {
			byte[] fieldvalue = getInputRowMeta().getBinary(r, data.fieldnrs[i]);
			for (int j = 0;j<fieldvalue.length;j++)
			list.add(fieldvalue[j]);
//			aryLen = fieldvalue.length;
//			System.arraycopy(fieldvalue,0,result,destLen,aryLen);
//			destLen += aryLen; 
		}
