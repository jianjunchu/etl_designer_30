...
//	        		data.bodyParameters[i].setValue(data.inputRowMeta.getString(rowData,data.body_parameters_nrs[i]));
	        		...
	        		data.bodyParameters[i].setValue(URLEncoder.encode(data.inputRowMeta.getString(rowData,data.body_parameters_nrs[i]),"UTF-8"));
		        	if(isDebug()) logDebug(BaseMessages.getString(PKG, "HTTPPOST.Log.BodyValue",data.bodyParameters[i].getName(),URLEncoder.encode(data.inputRowMeta.getString(rowData,data.body_parameters_nrs[i]),"UTF-8")));
		        }
            ...
            try
            {
                ...
                if( statusCode != -1 )
                {
                    ...
                    if (statusCode != 401) { 
                        ...
                        if(post.getResponseHeader("Content-Type")!=null)
                        	contentType = post.getResponseHeader("Content-Type").getValue();
                        ...
                        {
                      	  meta.setBinaryResult(true);
                      	  System.out.println("set http post to binary content!");
                        }
                ...
                if (!Const.isEmpty(meta.getFieldName())) {
                	if(meta.isBinaryResult())
                		newRow=RowDataUtil.addValueData(newRow, returnFieldsOffset, binaryBody);
                	else
