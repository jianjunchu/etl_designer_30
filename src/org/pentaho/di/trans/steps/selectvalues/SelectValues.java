...
public class SelectValues extends BaseStep implements StepInterface
{
	...
	*/
	...
	//			case ValueMetaInterface.TYPE_TIMESTAMP : rowData[index] = fromMeta.getTimestamp(rowData[index]); break;//tony 20171114
	...
	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException
	{
		...
		if (first)
		{
			...
			// padding char
			data.padChar = new String[meta.getPadChar().length];
			for (int i = 0; i < meta.getPadChar().length; i++) {
				data.padChar[i] = meta.getPadChar()[i];
			}
		}
