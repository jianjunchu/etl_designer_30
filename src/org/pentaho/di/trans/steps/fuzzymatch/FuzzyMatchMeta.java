...
public class FuzzyMatchMeta extends BaseStepMeta implements StepMetaInterface,StepMetaInjectionInterface
{
	...
	public final static int OPERATION_TYPE_CONTAINS= 10;
        ...
        for (StepInjectionMetaEntry entry : metadata) {
          KettleAttributeInterface attr = findAttribute(entry.getKey());
	          ...
	          if (entry.getValueType()!=ValueMetaInterface.TYPE_NONE) {
	        	if (attr.getKey().equals("lookupfield")) 
	        	{ 
	        			this.lookupfield = entry.getValue().toString();
	        	} else if (attr.getKey().equals("mainstreamfield")) {
	        		this.mainstreamfield = entry.getValue().toString();
	        	} else
	            { 
