...
public class PDFFileReaderMeta extends BaseStepMeta implements StepMetaInterface
{
	...
	public static final long RESTRICT_MAX_LINE = 50;
	...
	//jason
	/** Flag indicating that a file content should be included in the output */
	...
	public static String VERSION_RELEASE_FREE = "Version 1.0 ( Restricted )";
	public static String VERSION_RELEASE = "Version 1.0";//release version
	
	private String versionName = VERSION_RELEASE_FREE;
	
	public PDFFileReaderMeta()
	{
    ...
    //jason 
    /** 
     * @return Returns the includeContent.
     */
  ...
  //jason end
	...
	public Object clone()
	{
		PDFFileReaderMeta retval = (PDFFileReaderMeta) super.clone();
	...
	public void getFields(RowMetaInterface row, String name, RowMetaInterface[] info, StepMeta nextStep, VariableSpace space) throws KettleStepException
	{
		...
		if (includeContent)
		{
			ValueMetaInterface page = new ValueMeta("page", ValueMeta.TYPE_INTEGER);
			page.setOrigin(name);
			row.addValueMeta(page); 
			
			ValueMetaInterface content = new ValueMeta("content", ValueMeta.TYPE_STRING);
			content.setOrigin(name);
			row.addValueMeta(content); 
		}
		//jason end
	...
	public String[] getFilePaths(VariableSpace space)
	{
		...
		{
			defaultMask= new String[fileMask.length];
			for(int i=0;i< fileMask.length;i++)
			{
				try{
					if(Const.isEmpty(fileMask[i]) && new File(fileName[i]).isDirectory())
						defaultMask[i] =".*\\.[p|P][d|D][f|F]$";
					else
						defaultMask[i] =  fileMask[i];
				}catch(Exception ex){}			
			}
		}
		return FileInputList.createFilePathList(space, fileName, defaultMask,null, fileRequired);
	}
	...
	public FileInputList getFileList(VariableSpace space)
	{
		...
		//return FileInputList.createFileList(space, fileName, defaultMask, fileRequired, buildSubdirsFlagsArray(), buildFileTypeFiltersArray());
		for (int i=0;i<fileRequired.length;i++) fileRequired[i]="N";
		return FileInputList.createFileList(space, fileName, defaultMask, fileRequired, buildSubdirsFlagsArray());
	}
	
	public FileInputList getDynamicFileList(VariableSpace space, String[] filename, String[] filemask, String[] filerequired)
	{
		return FileInputList.createFileList(space, filename, filemask, filerequired, buildSubdirsFlagsArray());
	}
  ...
  public List<ResourceReference> getResourceDependencies(TransMeta transMeta, StepMeta stepInfo) {
	...
	{
		return new PDFFileReader(stepMeta, stepDataInterface, cnr, transMeta, trans);
	}

	public StepDataInterface getStepData()
	{
		return new PDFFileReaderData();
	}	
	...
	public String exportResources(VariableSpace space, Map<String, ResourceDefinition> definitions, ResourceNamingInterface resourceNamingInterface, Repository repository) throws KettleException {
		try {
			...
			if (!filefield) {
				...
				FileInputList fileList = FileInputList.createFileList(space, fileName, fileMask,null, fileRequired);
