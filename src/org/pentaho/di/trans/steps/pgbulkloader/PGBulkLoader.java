...
public class PGBulkLoader extends BaseStep implements StepInterface
{
	...
	public boolean execute(PGBulkLoaderMeta meta, boolean wait) throws KettleException
	{
        ...
        try  
        {
			...
			data.pgOutputStream = data.psqlProcess.getOutputStream(); //new FileOutputStream(new File("PGBulkLoader.txt"));
	...
	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException
	{
		...
		try
		{
			...
			{
				log.logBasic("write \\.");
				data.pgOutputStream.write("\\.".getBytes());
	...
	public boolean init(StepMetaInterface smi, StepDataInterface sdi)
			{
				...
				if (super.init(smi, sdi))
				{
					...
					String schemaName = environmentSubstitute(meta.getSchemaName());
					String tableName = environmentSubstitute(meta.getTableName());

					String schemaTable = StringUtil.isEmpty(schemaName)? tableName : schemaName+"."+tableName;
					try {
						database.connect();
						log.logBasic("Check the table is locked.");
						String checkSql = "";
						...
						if(meta.getFieldTable() ==null || meta.getFieldTable().length ==0){
							...
							String sql = "SELECT *"+Const.CR+"FROM "+ schemaTable +Const.CR; //$NON-NLS-1$ //$NON-NLS-2$
							RowMetaInterface rowMetaInterface = database.getQueryFields(sql,false);
							rowMetaInterface.getValueMetaList();
							int size = rowMetaInterface.size();
							meta.setFieldTable(new String[size]);
							meta.setFieldStream(new String[size]);
							meta.setDateMask(new String[size]);
							for(int i = 0 ; i < size ;i++){
								String name = rowMetaInterface.getValueMeta(i).getName();
								meta.getFieldTable()[i] =name;
								meta.getFieldStream()[i] = name;
								meta.getDateMask()[i]="";
							}
