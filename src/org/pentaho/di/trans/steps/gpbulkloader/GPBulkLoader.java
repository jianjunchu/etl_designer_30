		...
		if(meta.getFieldTable() ==null || meta.getFieldTable().length ==0){
			...
			try {
				...
				String schemaTable = meta.getDatabaseMeta().getQuotedSchemaTableCombination(environmentSubstitute(meta.getSchemaName()),
						environmentSubstitute(meta.getTableName()));
				RowMetaInterface rowMetaInterface = database.getTableFields(schemaTable);
				...
				for(int i = 0 ; i < size ;i++){
					...
					if(rowMetaInterface.getValueMeta(i).isDate()){
						meta.getDateMask()[i] = PGBulkLoaderMeta.DATE_MASK_DATETIME;
					}else{
						meta.getDateMask()[i]="";
					}
				}
